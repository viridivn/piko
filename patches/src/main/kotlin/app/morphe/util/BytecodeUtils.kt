...
fun BytecodePatchContext.traverseClassHierarchy(targetClass: MutableClass, callback: MutableClass.() -> Unit) {
    ...
    val superclassType = targetClass.superclass ?: return

    try {
        val superClassDef = classDefBy(superclassType)
        val mutableSuperClass = mutableClassDefBy(superClassDef)
        traverseClassHierarchy(mutableSuperClass, callback)
    } catch (_: Exception) {
    }
}
